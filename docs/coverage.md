...
| --- | --- |
| Document model and project view | `image`, `command`, `environment`, `extra_hosts`, `user`, `userns_mode`, `group_add`, `working_dir`, `read_only`, `healthcheck`, `depends_on`, `ports`, `volumes`, `networks`, `profiles`, `configs`, `secrets` |
